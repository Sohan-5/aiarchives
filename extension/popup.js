...
chrome.tabs.query(
	{ active: true, currentWindow: true, url: 'https://bard.google.com/*' },
	(tabs) => {
		if (tabs?.length) {
...
chrome.tabs.query(
	{ active: true, currentWindow: true, url: 'https://www.meta.ai/*' },
	(tabs) => {
		if (tabs?.length) {
			chrome.tabs.sendMessage(
				tabs[0].id,
...
chrome.tabs.query(
	{ active: true, currentWindow: true, url: 'https://claude.ai/*' },
	(tabs) => {
		if (tabs?.length) {
			chrome.tabs.sendMessage(
				tabs[0].id,
...
chrome.tabs.query(
	{ active: true, currentWindow: true, url: 'https://www.perplexity.ai/*' },
	(tabs) => {
		if (tabs?.length) {
			chrome.tabs.sendMessage(
				tabs[0].id,
